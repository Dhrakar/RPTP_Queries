...
with assignments as (
    select nbrbjob_pidm pidm,
     ...
     where pos.nbrjobs_ecls_code in ('NR', 'XR', 'CR', 'EX', 'NX', 'XX', 'FR', 'F9', 'FN', 'AR', 'A9') -- Historical Employee Classifications for Cumulative Service
       -- and nbrbjob_pidm =
...
--           busr.gobtpac_external_user || '@alaska.edu' "Supervisor Email",
    ...
    -- comment out since it is the old supervisor method
...
--      left join gobtpac busr on sup.ner2sup_sup_pidm = busr.gobtpac_pidm
    ...
    -- comment out NER2SUP since it is the old supervisor method
