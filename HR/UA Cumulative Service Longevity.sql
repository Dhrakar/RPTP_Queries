...
group by pidm)
    ...
    select :Calendar_Year_YYYY "Longevity Year",
          ...
          --  busr.gobtpac_external_user || '@alaska.edu' "Supervisor Email",
           service.yos "Cumulative Service Years",
           case
             when trunc(service.yos) != 0
              and remainder(trunc(service.yos), 5) = 0 then 'Y'
             else 'N'
           end "Award Year",
           round(((to_date('31-DEC-' || :Calendar_Year_YYYY, 'DD-MON-YYYY') - emp.pebempl_adj_service_date) / 365.25), 1) "Adj. Service Years",
